...
import { add, del, edit, toggleStatus } from './store/reducers/table-user/tableSlice';
...
export default function App() {
  ...
  const handleAddUser = () => {
    ...
    if (name && desc && email && phone && city) {
      ...
      setFormData({ name: '', desc: '', email: '', phone: '', city: '', status: 'Active' });
      setModalOpen(false);
    }
  };
  ...
  const allTajikistanCities = [
    "Dushanbe", "Khujand", "Bokhtar", "Kulob", "Isfara", "Istaravshan",
    "Tursunzoda", "Vahdat", "Hissar", "Panjakent", "Khorugh", "Konibodom",
    "Rogun", "Nurek", "Asht", "Shahriston", "Rasht", "Farkhor", "Danghara",
    "Yovon", "Baljuvon", "Jilikul", "Qumsangir", "Zafarobod"
  ];
    ...
    <div className="container">
          ...
          {filteredData.length > 0 ? (
            filteredData.map((user) => (
              <tr key={user.id}>
                ...
                <td>
                  <Button size="small" onClick={() => dispatch(del(user.id))} color="error">
                    <DeleteIcon />
                  </Button>
                  <Button
                    size="small"
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <Box sx={style}>
          ...
          <Button fullWidth variant="contained" onClick={handleAddUser} sx={{ mt: 2 }}>
            Save
          </Button>
        </Box>
      </Modal>
      ...
      <Modal open={editModalOpen} onClose={() => setEditModalOpen(false)}>
        <Box sx={style}>
          ...
          <Button fullWidth variant="contained" onClick={handleEditUser} sx={{ mt: 2 }}>
            Save Changes
          </Button>
        </Box>
      </Modal>
        ...
        <Box sx={style}>
          ...
          {selectedUser && (
            <>
              ...
              <p><strong>Description:</strong> {selectedUser.desc}</p>
              <p><strong>Email:</strong> {selectedUser.email}</p>
              <p><strong>Phone:</strong> {selectedUser.phone}</p>
              <p><strong>City:</strong> {selectedUser.city}</p>
              <p><strong>Status:</strong> {selectedUser.status}</p>
              <p><strong>Done:</strong> {selectedUser.done ? 'Yes' : 'No'}</p>
            </>
          )}
          <Button fullWidth variant="outlined" onClick={() => setInfoModalOpen(false)} sx={{ mt: 2 }}>
            Close
          </Button>
        </Box>
      </Modal>
